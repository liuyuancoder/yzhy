...
class Mysqls{
	...
	private $config = array('host'=>'localhost','username'=>'root','password'=>'cibnet','dbname'=>'kh');//当前使用
	//private $mconfig = array('host'=>DB_HOST,'username'=>DB_USERNAME,'password'=>DB_PASSWORD,'dbname'=>DB_NAME);//主数据库
	//private $sconfig = array('host'=>DB_SLAVE_HOST,'username'=>DB_SLAVE_USERNAME,'password'=>DB_SLAVE_PASSWORD,'dbname'=>DB_SLAVE_NAME);//slave数据库
	...
	function __construct($is_slave=false,$p=array()){
		...
		//if($is_slave && $this->sconfig['slave_host']){//选择slave数据库并且配置了slave数据库
